...
class DataHelper(object):
    ...
    def create_data_helper(self, data_dir, config, coref_trainer):
        ...
        for i, rst_tree in enumerate(self.read_rst_trees(data_dir=data_dir)):
            ...
            # tok_edus = [nltk.word_tokenize(edu) for edu in doc.doc_edus]  # RST-DT
            tok_edus = [edu.split(" ") for edu in doc.doc_edus]     # GUM
        ...
        # dev_sub_dir_name = str(data_dir).split("/")[-1].strip()
        dev_path = '../data/gum_dev/'
        self.val_trees = [os.path.join(dev_path, f) for f in os.listdir(dev_path) if f.endswith(".dis")]
        self.all_clusters = []
