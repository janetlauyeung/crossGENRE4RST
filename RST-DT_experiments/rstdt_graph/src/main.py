...
def parse_args():
    ...
    parser.add_argument('--prepare', action='store_true', help='whether to extract feature templates, action maps and relation maps')
    parser.add_argument('--train', action='store_true', help='whether to train new models')
    parser.add_argument('--eval', action='store_true', help='whether to do evaluation')
...
def get_train_loader(data_helper, config):
    ...
    action_labels = list(zip(data_helper.actions_numeric, data_helper.relations_numeric))  # a list of tuples

    train_data = RstDatasetCoref(action_feats, action_labels, data_helper, is_train=True)
    # train_data.X: 2878
    # train_data.y: 2878
    # len(action_feats): 2878
...
def train_model_coref(data_helper, config):
    ...
    os.makedirs("../data/model", exist_ok=True)     # make model dir if doesn't exist
...
if __name__ == '__main__':
    ...
    # helper_name = "data_helper_rst.bin"
    # helper_name = f"{config[MODEL_NAME]}_data_helper_rst.bin"       #  incorporate model name into data helper
    ...
    if args.eval:
        ...
        with torch.no_grad():
            ...
            evaluator.eval_parser(None, path=args.eval_dir, save_preds=True, report=True, validation=False, use_parseval=True)
