...
## Author: Amir Zeldes
## Date: 2020-02-09
## Notes: Adapted and revised by Janet Liu 
...
def debinarize(node, parent_dict, all_nodes):
	...
	for child_id in child_ids:
		...
		try:  # updated on Feb 18 2022
			if child.child_rel.lower() in ["joint", "joint-disjunction", "joint-list", "joint-sequence", "joint-other", "restatement", "restatement-repetition", "same-unit", "cause",
			                               "contrast", "adversative", "adversative-contrast", "temporal", "evaluation", "condition", "comparison",
			                               "textual-organization", "topic-change", "topic-comment"] and node.type == "multinuc":
...
def to_rs3(text):
	...
	# updated on Feb 18 2022: fixed the Multiple ROOT issue in RSD resulting from dis2rs3
	if len(groups) >= 2:
		if groups[1].split(" ")[-1].split("=")[-1].replace("/>", "").replace('"', "") in multinuc_rels:
			groups[0] = '<group id="1000" type="multinuc" />'
