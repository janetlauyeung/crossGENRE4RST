...
def debinarize(node, parent_dict, all_nodes):
	...
	for child_id in child_ids:
		...
		try:  # modified 2022/02/17
			if child.child_rel.lower() in ["joint", "joint-disjunction", "joint-list", "joint-sequence", "joint-other",
			                               "sequence", "restatement", "restatement-repetition", "same-unit", "cause",
			                               "contrast", "adversative-contrast", "temporal", "evaluation", "condition", "comparison",
...
def to_rs3(text):
	...
	# updated on Feb 17 2022: fixed the Multiple ROOT issue in RSD resulting from dis2rs3
	if len(groups) >= 2:
